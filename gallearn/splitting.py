...
# Mirrors `tgt_sfr_dir` in src/Dataset.jl. Not config-driven: there is
# only ever one of these files, generated by scripts/gen_sfrs.jl.
AVG_SFR_CSV = pathlib.Path(
...
def write_split(
    ...
    SPLITS_DIR / 'split_<split_name-or-timestamp>.json'. write_split
    calls dataset_lock.verify_dataset(dataset_fname) before doing
    anything else, so it raises immediately if dataset_fname has no
    recorded hash lock yet or if its content has drifted since
    locking. Like SPLITS_DIR itself, the split's location and naming
    convention aren't caller-configurable, only split_name is:
    keeping every split discoverable in one place matters more than
    letting a caller scatter them under arbitrary paths.
    ...
    split_name : str, optional
        Used in the output filename in place of a timestamp.
    ...
    suffix = split_name if split_name else _now_iso().replace(':', '')
